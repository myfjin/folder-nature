...
def license_text(tier: str, licenses_dir: Optional[Path] = None) -> tuple[str, bool]:
...
def accept_license(customer_id: str, tier: str, *,
                   licenses_dir: Optional[Path] = None,
                   license_tier: Optional[str] = None) -> LicenseAcceptance:
...
class SaleRegistry:
    ...
    def save(self) -> None:
        ...
        self.path.write_text(
            json.dumps({"sales": list(self._by_number.values())},
...
def stamp_sale(src: Path, dst: Path, *, config: MarkConfig, customer_id: str,
               number: str, license: LicenseAcceptance,
               registry: SaleRegistry) -> SaleRecord:
    ...
    for p in sorted(dst.rglob("*")):
        if p.is_file() and is_supported(p):
            stamp_file(p, WatermarkPayload(
                trademark=config.trademark,
                company=config.company or config.trademark,
                number=number))

    sale = SaleRecord(number=str(number), customer_id=customer_id,
                      trademark=config.trademark, tier=config.tier,
                      license=license)
