...
from .config import (MarkConfig, MarkConfigError, TIERS, load_config, save_config)
...
def _resolve_config(target: Path, args: argparse.Namespace) -> Optional[MarkConfig]:
    ...
    if cfg is None and not trademark:
        return None
    if cfg is None:
        return MarkConfig(trademark=trademark, company=company,
                          tier=tier or "team")
    if trademark:
...
def _read_pubkey(val: Optional[str]) -> Optional[str]:
    if not val:
        return None
...
def cmd_trademark(args: argparse.Namespace) -> int:
    ...
    if args.set:
        try:
            cfg = MarkConfig(trademark=args.set, company=args.company,
                             tier=args.tier or "team", license_tier=args.license_tier)
    ...
    cfg = load_config(directory)
    if cfg is None:
        print(f"no mark config at {directory} — set one with "
              f"`folder-nature trademark . --set NAME`", file=sys.stderr)
        return 1
...
def cmd_stamp(args: argparse.Namespace) -> int:
    ...
    if cfg is None:
        return _err("no trademark configured — set one with `trademark --set` "
    ...
    stamped = skipped = 0
    for f in files:
        payload = WatermarkPayload(trademark=cfg.trademark,
        ...
        if ok:
            ...
            skipped += 1
    print(f"stamped {stamped} file(s), skipped {skipped} "
...
def cmd_mark_show(args: argparse.Namespace) -> int:
    ...
    if rep.payload:
        ...
        print(f"number:    {rep.payload.number}"
              f"{'  (MASTER original)' if rep.payload.is_master else ''}")
    print(f"channels:  present={rep.channels_present or '-'} "
...
def cmd_copy(args: argparse.Namespace) -> int:
    ...
    if cfg is None:
        return _err("no trademark configured at source — set one with "
                    "`trademark --set` or pass --trademark")
    try:
        result = copy_limiter.copy_tree(src, Path(args.dst).resolve(), cfg,
                                        number=args.number)
    except (FileExistsError, NotADirectoryError, MarkConfigError) as e:
        return _err(str(e))
    print(f"copied -> {result.dst}")
    print(f"  number {result.number}, stamped {result.files_stamped}, "
...
def cmd_keygen(args: argparse.Namespace) -> int:
    ...
    try:
        path = signing.write_private_key(seed_hex, out, allow_in_repo=args.allow_in_repo)
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return _err(str(e))
    if trusted is None:
        print("WARNING: no trusted public key given (--pubkey). Checking the "
...
def cmd_stamp_sale(args: argparse.Namespace) -> int:
    ...
    if not args.accepted:
        return _err("refusing to stamp a sale without --accepted "
    ...
    acceptance = sale_mod.accept_license(
        args.customer, cfg.tier, licenses_dir=licenses_dir,
        license_tier=cfg.license_tier or cfg.tier)
    registry = sale_mod.SaleRegistry(Path(args.registry))
    try:
        rec = sale_mod.stamp_sale(src, Path(args.dst).resolve(), config=cfg,
                                  customer_id=args.customer, number=str(args.number),
                                  license=acceptance, registry=registry)
    ...
    print(f"  license {rec.license.license_tier} accepted "
...
def add_mark_subcommands(sub) -> None:
    ...
    p.add_argument("--set", metavar="NAME", help="Set the trademark (required to write)")
    ...
    p = sub.add_parser("stamp", help="Embed a watermark (master=0 default) — attribution, not prevention")
    ...
    p = sub.add_parser("copy", help="Cooperative tiered copy with auto-numbering (honest customers)")
    p.add_argument("src")
    p.add_argument("dst")
    ...
    p = sub.add_parser("keygen", help="Generate an Ed25519 signing keypair (private key mode 600)")
    p.add_argument("--out", required=True, help="Path for the private key (outside the repo)")
    p.add_argument("--allow-in-repo", action="store_true",
                   help="Override the refusal to write a key inside a git repo")
    p.set_defaults(func=cmd_keygen)

    p = sub.add_parser("sign", help="Sign a directory (sha256 manifest + Ed25519) — origin + integrity")
    p.add_argument("dir")
    ...
    p.add_argument("--number", required=True, help="Per-customer watermark number (not 0)")
    ...
    p.add_argument("--licenses-dir", help="Directory of <tier>.txt license texts (optional)")
    p.add_argument("--trademark", help="Override/supply trademark")
    p.add_argument("--tier", choices=TIERS)
