...
def test_sale_requires_matching_accepted_license(tmp_path):
    ...
    with pytest.raises(sale_mod.EnforcementError):
        sale_mod.stamp_sale(src, tmp_path / "out", config=cfg, customer_id="acme",
                            number="5", license=wrong, registry=reg)
...
def test_master_number_is_not_a_sale(tmp_path):
    ...
    with pytest.raises(sale_mod.EnforcementError):
        sale_mod.stamp_sale(src, tmp_path / "out", config=cfg, customer_id="acme",
                            number="0", license=acc, registry=reg)
...
def test_full_sale_then_leak_traces_and_claims(tmp_path):
    ...
    sale_mod.stamp_sale(src, tmp_path / "sold", config=cfg, customer_id="acme-corp",
                        number="5", license=acc, registry=reg)
...
def test_duplicate_number_rejected(tmp_path):
    ...
    sale_mod.stamp_sale(src, tmp_path / "o1", config=cfg, customer_id="a",
                        number="5", license=acc, registry=reg)
    acc2 = sale_mod.accept_license("b", "team")
    with pytest.raises(ValueError):
        sale_mod.stamp_sale(src, tmp_path / "o2", config=cfg, customer_id="b",
                            number="5", license=acc2, registry=reg)
